...
function addAllergy($email, $profile_name, $allerg, &$addErr) {
    ...
    if (empty($allerg)) {
        ...
        if (mysqli_num_rows($addSearch) > 0) {
            ...
            $sql = "INSERT INTO `Allergy` (`Allergy_Name`, `Name`, `Email_ID`) Values ('$allerg_E', '$profile_name_E', '$email_E')";
...
function updateAllergy($email, $profile_name, $find_all, $up_all, &$manageErr) {
    $success = false;

    if (empty($up_all)) {
        $manageErr = "Error: Allergy name is blank";
    }
    else {
        // Create connection
        $con=mysqli_connect(DB_HOST,DB_USER,DB_PASS,DB_NAME);
        ...
        $email_E = mysqli_real_escape_string($con, $email);
        $profile_name_E = mysqli_real_escape_string($con, $profile_name);
        $up_all_E = mysqli_real_escape_string($con, $up_all);
        $upSearch = mysqli_query($con, "SELECT * FROM `Allergy` WHERE `Email_ID`='$email_E' AND `Name`='$profile_name_E' AND `Allergy_Name`='$up_all_E'");
        if (mysqli_num_rows($upSearch) > 0) {
            $manageErr = "Error: This allergy already exists in the profile";
        }
        else {
            $find_all_E = mysqli_real_escape_string($con, $find_all);
            $sql = "UPDATE `Allergy` SET `Allergy_Name`='$up_all_E' WHERE `Email_ID`='$email_E' AND `Name`='$profile_name_E' AND `Allergy_Name`='$find_all_E'";
            if (!mysqli_query($con,$sql)){
                die('Error: ' . mysqli_error($con));
            }
...
function deleteAllergy($email, $profile_name, $find_all) {
    ...
    $profile_name_E = mysqli_real_escape_string($con, $profile_name);
    $find_all_E = mysqli_real_escape_string($con, $find_all);
    // delete all Restaurants, then delete the Account
...
function updateProfile($email, $profile_name, $up_name, &$upErr) {
    ...
    if (empty($up_name)) {
        ...
        $upSearch = mysqli_query($con, "SELECT * FROM `Allergy` WHERE `Email_ID`='$email_E' AND `Name`='$up_name_E'");
        if (mysqli_num_rows($upSearch) > 0) {
