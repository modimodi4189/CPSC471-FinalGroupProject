...
function addIng($res_ID, $new_name, $new_type, $new_quan, &$addErr) {
    ...
    if (empty($new_name)) {
        ...
        $addSearch = "SELECT * FROM (`Amount_left` NATURAL JOIN `Ingredient`)
        ...
        if (mysqli_num_rows($addSearch) > 0) {
            ...
            $new_id_E = mysqli_real_escape_string($con, mysqli_insert_id($con));
            $new_quan_E = mysqli_real_escape_string($con, $new_quan);

            $sql = "INSERT INTO `Amount_left` (`IngredientID`, `RestaurantID`, `Quantity`) VALUES ('$new_id_E', '$res_ID_E', '$new_quan_E')";
            if (!mysqli_query($con,$sql)) {
                die('Error: ' . mysqli_error($con));
            }
...
function updateIng($res_ID, $find_IID, $up_quan, &$manageErr) {
    ...
    if (empty($up_quan) || $up_quan < 0) {
        ...
        $sql = "UPDATE `Amount_left` SET `Quantity`='$up_quan_E' WHERE `RestaurantID`='$res_ID_E' AND `IngredientID`='$find_IID_E'";
        if (!mysqli_query($con,$sql)){
            die('Error: ' . mysqli_error($con));
        }
