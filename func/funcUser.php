        ...
        if (mysqli_num_rows($addSearch) > 0) {
            ...
            foreach ($alls as $allerg):
                if (!empty($allerg)) {
                    ...
                    $find_result = mysqli_query($con, "SELECT * FROM `Allergy` WHERE `Email_ID`='$email_E' AND `Name`='$add_name_E' AND `Allergy_Name`='$allerg_E'");
                    if (mysqli_num_rows($find_result) == 0) {
                        $sql = "INSERT INTO `Allergy` (`Allergy_Name`, `Name`, `Email_ID`) Values ('$allerg_E', '$add_name_E', '$email_E')";
                        if (!mysqli_query($con,$sql)){
                            die('Error: ' . mysqli_error($con));
                        }
                    }    
