    ...
    if (mysqli_num_rows($man_result) == 0) {
        ...
        while ($find_row = mysqli_fetch_array($find_result)) {
            ...
            if (mysqli_num_rows($find_dish_ing) > 0) {
                ...
                foreach ($_SESSION["cur_Allergies"] as $allergy) {
                    $allergy_E = mysqli_real_escape_string($con, $allergy);
                    // find matching ingredient name OR type
                    $find_allergy = mysqli_query($con, "SELECT * FROM (`Ingredient` NATURAL JOIN `Needs`) WHERE `Dish_ID`='$cur_dishID_E'
                                                        AND (`IngredientName`='$allergy_E' OR `Type`='$allergy_E')");
                    if (mysqli_num_rows($find_allergy) > 0) {
                        $foundAllergy = 1; // found Allergy
                        break;
                    }
                }
                ...
                if ($foundAllergy == 0) {
                    ...
                    $ing_quan = -1;
                    $find_ings = mysqli_query($con, "SELECT * FROM (`Ingredient` NATURAL JOIN `Amount_left` NATURAL JOIN `Needs`) WHERE `Dish_ID`='$inserted_dish_E'");
...
function makeOrder($order_res_ID, $order_res_name, &$orderErr) {
    ...
    if (empty($order_dishID)) {
        ...
        for ($i = 0; $i < count($order_dishID); $i++) {
            ...
            $find_result = mysqli_query($con, "SELECT * FROM (`Ingredient` NATURAL JOIN `Amount_left` NATURAL JOIN `Needs`)
            ...
            if (mysqli_num_rows($find_result) == 0) {
                ...
                while ($find_row = mysqli_fetch_array($find_result)) {
                    ...
                    $sql = "UPDATE `Amount_left` SET `Quantity`='$cur_ing_new_E' WHERE `IngredientID`='$ing_ID_E'";
                    if (!mysqli_query($con,$sql)){
                        die('Error: ' . mysqli_error($con));
                    }
                }
