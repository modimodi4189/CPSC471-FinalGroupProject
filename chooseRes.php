    ...
    if ($_POST["Sel_Prof"]) {
        if (!isset($_POST["profRadio"])) {
            ...
            if (mysqli_num_rows($man_result) == 0) {
                ...
                $man_result = mysqli_query($con, "SELECT * FROM `Allergy` WHERE `Email_ID`='$email_E' AND `Name`='$sel_pro_E'");
                if (mysqli_num_rows($man_result) > 0) {
                    $_SESSION["cur_Allergies"] = array();
            
                    while ($find_row = mysqli_fetch_array($man_result)) {
                        $_SESSION["cur_Allergies"][] = $find_row["Allergy_Name"];
                    }    
                }
